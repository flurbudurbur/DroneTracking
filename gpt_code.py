...
class FaceDetector:
    ...
    def start(self):
        ...
        while True:
            frame = cv.cvtColor(self.drone.get_frame_read().frame, cv.COLOR_RGB2BGR)
            ...
            if len(detections) > 0:
                diff = self.__track_faces(frame, detections)
            ...
            cv.imshow('Cam girls :3', cv.cvtColor(frame, cv.COLOR_BGR2RGB))
    ...
    def __connect_stream(self):
        try:
            ...
            if response.find('ok') != 0:
                ...
                print('Successfully started drone camera')
        except DroneException as de:
            exit(de)
    ...
    def __draw_detections(self, frame, x, y, w, h, cx):
        ...
        r = 255 if label == 'Unknown' else 0
        g = 255 if label != 'Enemy' else 0
        b = 255 if label == 'Enemy' else 0
        ...
        return label, colored_mask, diff
    ...
    def __track_faces(self, frame, detections):
        ...
        for x, y, w, h in detections:
            cx = int((x + w) / 2)
            cy = int((y + h) / 2)
            label, mask, diff = self.__draw_detections(frame, x, y, w, h, cx)
            self.tracking_lost_count = 0
    ...
    def __r_left(self, val):
        try:
            ...
            if response.find('ok') != 0:
                ...
                print('Successfully rotated left')
        except DroneException as de:
            print(de)
    ...
    def __r_right(self, val):
        try:
            ...
            if response.find('ok') != 0:
                ...
                print('Successfully rotated right')
        except DroneException as de:
            print(de)
